...
import { FiUsers, FiUser, FiHome, FiAlertCircle, FiChevronRight, FiChevronDown, FiBarChart2 } from "react-icons/fi";
...
interface ThanaIOStat {
    thanaName: string;
    ioStats: IOStat[];
    noIoCount: number;
}

interface TIResponse {
    thana: string;
    ioStats: IOStat[];
    noIoCount: number;
}
        ...
        const fetchStats = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    toast.error("डेटा प्राप्त करने में विफल");
                }
            } catch (error) {
                console.error("Fetch stats error:", error);
                toast.error("सर्वर त्रुटि");
            } finally {
                setLoading(false);
            }
        };
            ...
            {isTI ? (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <StatCard
                            ...
                            value={stats?.ioStats?.length || 0}
                            icon={<FiUsers />}
                            color="blue"
                        />
                        <StatCard
                            title="आवंटित मामले"
                            value={stats?.ioStats?.reduce((acc: number, curr: any) => acc + curr.count, 0) || 0}
                            icon={<FiBarChart2 />}
                            color="emerald"
                        />
                        <StatCard
                            title="बिना आवंटित (No IO)"
                            value={stats?.noIoCount || 0}
                            icon={<FiAlertCircle />}
                            color="orange"
                        />
                    </div>
                                                ...
                                                <div className="flex items-center justify-end gap-3">
                                                    <div className="h-2 w-32 rounded-full bg-gray-100 overflow-hidden">
                                                        <div
                                                            ...
                                                            style={{ width: `${Math.min((io.count / (stats.ioStats.reduce((a: number, c: any) => a + c.count, 0) || 1)) * 100, 100)}%` }}
                                                        ></div>
                                                    </div>
                                                    <span className="text-xs font-medium text-gray-400">
                                                        {Math.round((io.count / (stats.ioStats.reduce((a: number, c: any) => a + c.count, 0) || 1)) * 100)}%
                                                    </span>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                                    {stats?.ioStats?.length === 0 && (
                                        <tr>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {stats?.map((thana: ThanaIOStat) => (
                            <div
                                key={thana.thanaName}
                                className={`rounded-2xl border transition-all duration-300 ${expandedThana === thana.thanaName ? 'border-blue-200 ring-2 ring-blue-50' : 'border-gray-100'} bg-white shadow-sm overflow-hidden`}
                            >
                                <button
...
const StatCard = ({ title, value, icon, color }: { title: string, value: string | number, icon: React.ReactNode, color: 'blue' | 'emerald' | 'orange' }) => {
    ...
    return (
        <div className={`flex items-center gap-5 rounded-2xl border bg-white p-6 shadow-sm transition-all hover:shadow-md`}>
            <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border ${colors[color]}`}>
